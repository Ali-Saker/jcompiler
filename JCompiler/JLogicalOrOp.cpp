...
JLogicalOrOp::JLogicalOrOp(int line, int col, JExpression *lhs, JExpression *rhs)
: JBoolBinaryExpression(line, col, lhs, rhs, "||")
{
}
void JLogicalOrOp::preAnalyze(Context *surrounding){
	if (this->lhs != 0){
...
void JLogicalOrOp::codegen(){
	this->rhs->codegen();
	this->lhs->codegen();

	Utils::out << "lw $t1,8($sp)" << endl;
	Utils::out << "lw $t2,4($sp)" << endl;
	Utils::out << "or $t3,$t1,$t2" << endl;
